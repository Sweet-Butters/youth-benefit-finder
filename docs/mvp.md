...
flowchart LR
    ...
    B -->|"결과 이미지 공유"| A
    B -->|"계속 받아보기"| C["3. 모으기<br/>카톡 채널·인스타 팔로우"]
    C -->|"매주 새 콘텐츠"| A
...
- 질문 5개(관심 분야, 지금 상황, 나이, 지역, 쓸 수 있는 시간)에 답하면 → **갈 수 있는 여러 길 + 이번 주에 할 일 1개 + 받을 수 있는 지원**을 보여 준다.
- 로그인 없이 쓴다. 답한 내용은 서버에 저장하지 않는다.
...
- **회원가입을 만들지 않는다.** 미성년자 개인정보를 저장하면 보호자 동의(만 14세 미만)와 관리 책임이 생긴다.
- **카카오톡 채널 친구 + 인스타 팔로워**를 '모인 사람'으로 센다. 매주 새 콘텐츠를 보낸다.
...
| 9 | 청소년이 받을 수 있는 교육·자격증·훈련 지원 한눈에 | 지원 정보 | 원문 링크와 확인 날짜 필수 |
...
| **남았나 (가장 중요)** | 카톡 채널 친구 + 인스타 팔로워, 다시 찾아온 비율 | 채널·인스타 통계 |
...
| 기준 | 넘어가면 |
|---|---|
| 카톡 채널 친구 + 인스타 팔로워 **1,000명** | 멘토 연결 소규모 시험 (센터 선생님을 통해 5~10쌍) |
...
- 로그인, 회원 데이터베이스
- 멘토 연결 기능
- 지원 정보 자동 수집 (도구에 쓸 지원 정보는 손으로 30~50개)
